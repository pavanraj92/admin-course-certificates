...
class DebugCertificatesCommand extends Command
{
    ...
    protected $description = 'Debug Certificates module loading';

    public function handle()
    {
        $this->info('🔍 Debugging Certificates Module...');
        
        // Check which route file is being loaded
        $this->info("\n📍 Route Loading Priority:");
        $moduleRoutes = base_path('Modules/Certificates/routes/web.php');
        $packageRoutes = base_path('packages/admin/certificates/src/routes/web.php');
        ...
        $viewPaths = [
            'Module views' => base_path('Modules/Certificates/resources/views'),
            'Published views' => resource_path('views/admin/certificate'),
            'Package views' => base_path('packages/admin/certificates/resources/views'),
        ];
        ...
        $controllerClass = 'Modules\\Certificates\\app\\Http\\Controllers\\Admin\\CertificateController';
        ...
        foreach ($routes as $route) {
            ...
            if (isset($action['controller']) && str_contains($action['controller'], 'CertificateController')) {
                $certificateRoutes[] = [
                    'uri' => $route->uri(),
        ...
        if (!empty($certificateRoutes)) {
            $this->table(['URI', 'Methods', 'Controller', 'Name'], $certificateRoutes);
        } else {
            $this->warn("No certificate routes found.");
        }
    }
}
